...
describe("buildCoverageHint", () => {
  it("Land empfohlen, aber nicht abgedeckt: ehrlicher Fallback ohne Gedankenstrich", () => {
    const r = lookupPLZWithLevel("50667");
    const hint = buildCoverageHint(
      { ...r, coverage: { ...r.coverage, landSupported: false } },
      "Land"
    );
    expect(hint).toContain("können wir die Landesebene noch nicht sicher zuordnen");
